...
class NotificationTools:
    ...
    def _get_batch_sizes(self) -> Dict:
        ...
        try:
            ...
            LOGGER.warning(
                "无法读取 advanced.batch_size（%s: %s），使用默认批次大小 / "
                "could not read advanced.batch_size (%s: %s); using defaults",
    ...
    def _get_batch_interval(self) -> float:
        ...
        try:
            ...
            LOGGER.warning(
                "无法读取 advanced.batch_send_interval（%s: %s），使用默认间隔 %s / "
                "could not read advanced.batch_send_interval (%s: %s); using default %s",
                config_path, e, _BATCH_INTERVAL_DEFAULT,
                config_path, e, _BATCH_INTERVAL_DEFAULT,
            )
