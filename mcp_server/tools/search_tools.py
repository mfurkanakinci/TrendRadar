        ...
        try:
            ...
            while current_date <= search_end:
                try:
                    ...
                    skipped_dates.append({"date": date_str, "error": str(e)})
                    logger.warning(
                        "search_related_news_history: skipping date %s (reference=%r): %s",
                        date_str, reference_title, e, exc_info=True,
                    )
            ...
            if not all_related_news:
                ...
                if skipped_dates:
                    empty_result["partial"] = True
                    empty_result["skipped_dates"] = skipped_dates
                return empty_result
